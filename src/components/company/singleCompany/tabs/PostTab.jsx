...
export default function PostTab({ posts, userData }) {
  ...
  return (
    <div className="relative">
      <Swiper
        ...
        className="px-1 !pt-5 !pb-10 sm:px-2 md:px-4"
      >
        {postsList.map((post) => (
          <SwiperSlide
            key={post._id}
  ...
  return (
    <>
      <LikeUserModel
        ...
        post={localPost}
      />
      <div className="rounded-[5px] bg-white h-[552px] flex flex-col mx-4 pt-4">

        {/* User/Company Info */}
        <div className="flex items-center gap-2.5 border-b border-black/10 px-[17px] py-[17px]">
          <div className="relative h-10 w-10">
            <ImageFallback
              src={userData?.logoUrl && getImg(userData?.logoUrl || noImage2)}
              fallbackSrc={noImage2}
              width={40}
              height={40}
              priority={true}
              alt={userData?.companyName || `${userData?.firstName} ${userData?.lastName}` || "company"}
              className="mb-[25px] h-10 w-10 rounded-full object-cover"
              style={{ height: "auto" }}
            />
          </div>
          <div className="min-w-0 text-left">
            <p className="truncate text-[13px] font-medium leading-relaxed break-words">
